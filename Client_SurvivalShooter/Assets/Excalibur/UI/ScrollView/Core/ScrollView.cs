...
namespace Excalibur
{
    public abstract class ScrollView<TItemData, TContext> : MonoBehaviour where TContext : class, new ()
    {
        ...
        readonly IList<ScrollCell<TItemData, TContext>> pool = new List<ScrollCell<TItemData, TContext>> ();
        ...
        void ResizePool (float firstPosition)
        {
            ...
            {
                var cell = Instantiate (CellPrefab, cellContainer).GetComponent<ScrollCell<TItemData, TContext>> ();
                if (cell == null)
                {
    ...
    public abstract class ScrollView<TItemData> : ScrollView<TItemData, NullContext> { }
}
