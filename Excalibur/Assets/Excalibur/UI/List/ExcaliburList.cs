...
/*
 * 编辑器非播放模式下，排列item，item个数根据滑动方式固定
 * 单行/单列始终居中排列
 * 多行/多列默认从左上角开始排列，根据滑动方向，垂直滑动从从左到右，从上到下；水平滑动从上到下，从左到右
 * 水平、垂直滑动，会在相应方向上除了Mask内的item外，多生成两个item用于虚拟列表位置变化处理
 * ScrolllAxis.Horizontal       水平方向滑动
 * ScrolllAxis.Vertical         垂直方向滑动
 * ScrolllAxis.Arbitrary        会有一个content存放内容
 * 
 * Vertical     可以选择固定列数，如果不固定列数就根据Mask宽自动排列列数
 * Horizontal   可以选择固定行数，如果不固定行数就根据Mask高自动排列行数
 * 
 * ScrollType.Nothing    最多只会排列列表内所有能见的item个数。其他将会有多两列或者两排完全在Mask外的item来处理虚拟效果
 */
...
namespace Excalibur
{
        ...
        ISubject
    {
        ...
        public void CreateItems ()
        {
            if (!_prefab)
            {
                throw new NullReferenceException ("List Prefab 为空");
            }
